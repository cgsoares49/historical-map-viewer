...
class DataLoader {
    ...
    async loadTile(tile) {
        const [cst, pol, par, cities, iwa, niw, riv, ctr] = await Promise.all([
            ...
            this._fetchAndParse(tile.terrainFile,   this._parseTerrain.bind(this)),
        ]);
        return { cst, pol, par, cities, iwa, niw, riv, ctr };
    }
    ...
    // Parse CTR (Terrain contour lines) tile files.
    //
    // Format (modeled on RIV, but terrain isn't time-varying so there's no
    // date-range block):
    //   <poly_count>
    //   For each poly:
    //     <poly_type>  ,  <poly_index>   ← poly_type: 1 = regular contour,
    //                                       2 = index contour (5000ft multiples)
    //     <elevation_m>
    //     <point_count>
    //     <lon>  <lat>  (repeated)
    //
    // Returns: [ { polyType, polyIndex, elevation, points:[{lon,lat}] } ]
    _parseTerrain(text) {
        const lines = this._lines(text);
        if (!lines.length) return [];
        ...
        for (let p = 0; p < polyCount; p++) {
            ...
            const elevation = parseFloat(lines[i++]);
            ...
            polys.push({ polyType, polyIndex, elevation, points });
        }
        return polys;
    }
